...
cc.Class({
    ...
    SetUserInfo:function(userInfoNode,pid){
        ...
        let wechatSprite = userInfoNode.getChildByName("mask").getChildByName("head_img").getComponent(app.subGameName + "_WeChatHeadImage");
        wechatSprite.OnLoad();
    ...
    SetScore:function(nodeObjs,dataIndex){
        ...
        //比赛分
        let lb_sportsPointTitle = this.GetWndNode("fristFrame/title_list/lb_sportsPointTitle");
    ...
    //刷新单局信息
    UpdateScendFrameByIndex:function(){
        ...
        let data = eval('(' + this.recordList[this.curSetId - 1].dataJsonRes + ')');
        console.log("UpdateScendFrameByIndex data == " + data);
        this.backCode = this.recordList[this.curSetId - 1].playbackCode;
        for(let i=0;i<data.posResultList.length;i++){
            let player = data.posResultList[i];
            let playerNode = this.secondFrame.getChildByName("playerList").getChildByName("player" + (i + 1));
            ...
            for(let i = 0;i < allPlayer.length;i++){
                if(player.pid == allPlayer[i].pid){
                    playerInfo = allPlayer[i];
                    break;
                }
            }
            
            let head = playerNode.getChildByName("user_info").getChildByName("mask").getChildByName("head_img").getComponent("WeChatHeadImage");
    ...
    OnClick:function(btnName, btnNode){
        ...
        if('btn_close' == btnName){
            ...
            let selectSetId = parseInt(this.lb_go.string);
...
                    app[app.subGameName + "_SysNotifyManager"]().ShowSysMsg("您所在的亲友圈比赛分低于预警值，无法加入比赛，请联系管理");
    ...
    OnPack_VideoData:function(gameType){
        if(gameType == "DDZ"){
