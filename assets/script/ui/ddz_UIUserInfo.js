...
cc.Class({
    ...
    OnCreateInit: function () {
        ...
        let Gifts = app.ddz_SysDataManager().GetTableDict("Gift");
        this.giftCfg = [];
...
//        app.LocationMgr().setType("userinfo");
    ...
    ShowHero_NameOrID:function () {
        let heroName = app.wzmj_HeroManager().GetHeroProperty("name");
        this.label_name.string =heroName;
        let heroID = app.wzmj_HeroManager().GetHeroProperty("pid");
    ...
    ShowSex:function(){
    	let sex = -1;
        if(-1 == this.pos)
            sex = app.wzmj_HeroAccountManager().GetAccountProperty("Sex");
        else{
            let room = this.RoomMgr.GetEnterRoom();
